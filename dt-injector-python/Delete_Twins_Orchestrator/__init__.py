...
def main(req):
  ...
  OUTPUT_QUEUE_NAME = "delete-twin-queue"
  # Name of the container containing the specific blobs used in this function
  SPECIFIC_CONTAINER = "delete-storage/delete-twins"
  ...
  try:
      ...
      if(action != "Delete") :
        logging.exception("Dev Log: The action in request body doesn't match the triggered function : Delete_Twins_Orchestrator")
        return
  except:
  ...
  try:
      ...
      if(element != "Twin") :
        logging.exception("Dev Log: The element in request body doesn't match the triggered function : Delete_Twins_Orchestrator")
        return
  except:
